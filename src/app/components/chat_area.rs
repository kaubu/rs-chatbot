...
const OUTER_DIV_CLASS: &str = "h-screen pb-24 w-full flex flex-col\
overflow-y-auto border border-gray-300 rounded p-5 bg-gray-100";
...
pub fn ChatArea(
    ...
    view! {cx,
        <div class={OUTER_DIV_CLASS} node_ref=chat_div_ref>
            {move || conversation.get().messages.iter().map(move |message| {
                let class_str = if message.from_user {
                    USER_MESSAGE_CLASS
                } else {
                    MODEL_MESSAGE_CLASS
                };
